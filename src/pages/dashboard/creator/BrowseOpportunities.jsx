...
export default function BrowseOpportunities() {
  ...
  const handleCounter = async () => {
    ...
    const minAmount = Math.floor(counterModal.budget * 0.1);
    if (Number(counterAmount) < minAmount) {
      return showToast(
        `Minimum counter amount is PKR ${minAmount.toLocaleString()} (10% of budget)`,
      );
    }
    ...
    try {
      ...
      showToast(
        `✅ Counter offer of PKR ${Number(counterAmount).toLocaleString()} sent!`,
      );
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-purple-200 text-sm mt-1">
                    by{" "}
                    {expanded.brandId?.brandName ||
                      expanded.brandId?.fullName ||
                      expanded.brandName}
                  </p>
                </div>
                <button
                  onClick={() => setExpanded(null)}
                  className="w-10 h-10 rounded-xl bg-white/15 backdrop-blur-sm text-white hover:bg-white/30 flex items-center justify-center transition-all flex-shrink-0"
                >
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  {
                    label: "Budget",
                    value: `PKR ${expanded.budget?.toLocaleString()}`,
                    icon: "solar:wallet-money-bold",
                    color: "text-green-600",
                  },
                  {
                    label: "Platform",
                    value: expanded.platform,
                    icon:
                      platformIcons[expanded.platform] ||
                      "solar:smartphone-bold",
                    color: "text-purple-600",
                  },
                  {
                    label: "Deadline",
                    value: `${expanded.deadline} days`,
                  ...
                  <div
                    key={i}
                    className="bg-purple-50/60 border border-purple-100 rounded-2xl p-3 text-center"
                  >
                    <Icon
                      icon={item.icon}
                      className={`text-2xl mx-auto mb-1 ${item.color}`}
                    />
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center px-4">
          ...
          >
            <div className="bg-gradient-to-r from-purple-700 to-violet-700 p-5">
              ...
              <p className="text-purple-200 text-sm mt-1">
                Negotiate professionally with the brand.
              </p>
            ...
            <div className="p-6 space-y-4">
              <div className="bg-purple-50 rounded-2xl p-4 flex items-center justify-between">
                ...
                <span className="font-black text-purple-700 text-lg">
                  PKR {counterModal.budget?.toLocaleString()}
                </span>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mb-2">
                  <Icon
                    ...
                    className="text-purple-500 text-sm flex-shrink-0"
                  />
                  <p className="text-xs text-gray-500">
                    Min:{" "}
                    <span className="font-bold text-purple-600">
                      PKR{" "}
                      {Math.floor(counterModal.budget * 0.1).toLocaleString()}
                    </span>{" "}
                    (10% of budget)
                  </p>
                </div>
                <input
                  type="number"
                  placeholder={`Min PKR ${Math.floor(counterModal.budget * 0.1).toLocaleString()}`}
                  min={Math.floor(counterModal.budget * 0.1)}
                  value={counterAmount}
              ...
              <div className="flex gap-3 pt-1">
                <button
                  ...
                  className="flex-1 py-3 border-2 border-gray-200 text-gray-500 rounded-2xl text-sm font-semibold hover:border-purple-300 hover:bg-purple-50 hover:text-purple-600 transition-colors"
                >
                  Cancel
                </button>
                <button
          ...
          {[
            {
              ...
              label: `${filtered.length} Available`,
              color: "text-orange-500",
            },
            {
              icon: "solar:clock-circle-bold",
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 sm:pb-0">
          {availableCategories.map((cat) => (
            <button
              ...
              className={`whitespace-nowrap px-5 py-2.5 text-xs font-bold rounded-xl transition-all ${
                category === cat
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="bg-white rounded-3xl p-6 animate-pulse border border-purple-100 shadow-sm"
            >
        ...
        <div className="text-center py-20">
          <div className="w-20 h-20 bg-purple-50 rounded-3xl flex items-center justify-center mx-auto mb-4">
            <Icon icon="mynaui:search" className="text-4xl text-purple-300" />
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((op, i) => (
            ...
            >
              ...
              {/* Title */}
              <h3 className="font-black text-gray-900 text-[15px] mb-2 line-clamp-2 leading-tight">
                {op.title}
              </h3>
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex justify-between items-center text-xs">
                  <span className="text-gray-400 flex items-center gap-1">
                    <Icon
                      icon={
                        platformIcons[op.platform] || "solar:smartphone-bold"
                      }
              ...
              >
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-2.5 bg-gradient-to-r from-purple-600 to-violet-600 text-white text-sm font-bold rounded-xl hover:shadow-md hover:shadow-purple-200 transition-all disabled:opacity-60"
                >
                  {applying === op._id ? (
                    "..."
                  ) : (
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-2.5 border-2 border-purple-600 text-purple-600 text-sm font-bold rounded-xl hover:bg-purple-50 transition-colors"
                >
                  <>
                    <Icon
