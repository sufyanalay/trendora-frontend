...
export default function BrandCollaborations() {
  ...
  const handleRevision = async () => {
    ...
    try {
      await axios.put(`/collaborations/${revisionModal._id}/revision`, {
        revisionNote,
      });
      setCollaborations((prev) =>
        prev.map((c) =>
          c._id === revisionModal._id
            ? { ...c, status: "revision", revisionNote }
            : c,
        ),
      );
            ...
            {collaborations.map((c) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span
                    className={`text-xs font-semibold px-2.5 py-1 rounded-full flex-shrink-0 ml-2 ${statusColors[c.status]}`}
                  >
                ...
                {c.status === "payment_pending" && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-2 mb-3">
                    ...
                    <p className="text-xs text-yellow-600 mt-0.5">
                      Complete payment to unlock collaboration.
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-2">
                  ...
                  {(c.status === "payment_pending" ||
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleDeleteCollaboration(c._id);
                      }}
          ...
          {selected ? (
            <div className="bg-card rounded-2xl border border-border shadow-card flex flex-col h-[600px] sticky top-24">
              <div className="p-4 border-b border-border flex items-center gap-3">
                ...
                <span
                  className={`flex-shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full ${statusColors[selected.status]}`}
                >
