...
export default function MyOpportunities() {
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      setOpportunities(prev => prev.map(op =>
        op._id === editModal._id ? res.data : op
      ))
  ...
  const handleClose = async (id) => {
    if (!window.confirm('Close this opportunity? No new applications will be accepted.')) return
    try {
      await axios.put(`/opportunities/${id}/close`)
      setOpportunities(prev => prev.map(op =>
        op._id === id ? { ...op, status: 'closed' } : op
      ))
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center px-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-purple overflow-hidden">
            <div className="bg-gradient-to-r from-primary to-primary-dark p-5">
              <h3 className="font-black text-white text-lg">Edit Opportunity</h3>
              <p className="text-purple-200 text-sm mt-1 truncate">{editModal.title}</p>
            </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-4 text-xs text-muted">
                    ...
                    <span>⏰ {op.deadline} days</span>
                    <span>📁 {op.category}</span>
                    <span>📅 {new Date(op.createdAt).toLocaleDateString('en-PK', {
